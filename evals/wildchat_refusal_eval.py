"""
WildChat non-toxic refusal eval (post-hoc, base vs trained), Self-CTRL port.

Loads a deterministic subset of `allenai/WildChat-1M` filtered to first-turn
non-toxic English prompts, generates responses (Self-CTRL loaders), and uses a
Gemini judge to classify each response as a refusal or not. The headline metric
is `refusal_rate` — over-refusing on benign prompts is the failure mode this
catches (an over-trained safety policy that says no to ordinary requests).

The dataset is gated on Hugging Face — accept terms at
https://huggingface.co/datasets/allenai/WildChat-1M and authenticate with
`huggingface-cli login` before running.

Generation is sampled (temp=1.0, top_p=0.9) by default, matching the in-domain
training-time distribution. The judge call is greedy (temp=0).
"""
...
from evals.judges import gemini_text, XSTEST_JUDGE_PROMPT, classify_xstest_response
...
def load_wildchat_subset(max_prompts=300, seed=42, exclude_ids=None):
    ...
    print("Loading allenai/WildChat-1M (streaming, filter: first-turn / "
          "non-toxic / English)...")
    ...
    pbar.close()
    print(f"WildChat subset: {len(reservoir)} prompts (scanned {seen} rows, "
          f"seed={seed})")
    if len(reservoir) < max_prompts:
        print(f"  WARNING: only {len(reservoir)} eligible prompts found in "
...
def _judge_records(records, judge_model):
    def _judge_one(rec):
        prompt_text = XSTEST_JUDGE_PROMPT.format(
            question=rec["prompt"], response=rec["response"]
        )
        return gemini_text(prompt_text, model=judge_model)

    fails = 0
    pbar = tqdm(total=len(records), desc=f"judge/{judge_model}/wildchat", unit="resp")
    with ThreadPoolExecutor(max_workers=JUDGE_WORKERS) as pool:
        ...
        for fut in as_completed(futs):
            ...
            rec["classification"] = classify_xstest_response(judge_text)
            pbar.update(1)
            if fails:
                pbar.set_postfix(fails=fails)
    pbar.close()
...
def _build_metrics(records, judge_model, generation_backend):
    n = len(records)
    counts = {
        "full_compliance": sum(1 for r in records if r["classification"] == "full_compliance"),
        "partial_refusal": sum(1 for r in records if r["classification"] == "partial_refusal"),
        "full_refusal": sum(1 for r in records if r["classification"] == "full_refusal"),
    }
...
def _print_metrics(metrics):
    ...
    print("=" * 60)
    print(f"WildChat Refusal Results (n={metrics['n']}, "
    ...
    print(f"  3-way: full_compliance={c.get('full_compliance')} "
