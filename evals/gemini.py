...
# Concurrent in-flight Gemini calls. Every eval judges with the same quota, so
# this is set once here rather than per script.
JUDGE_WORKERS = 8
...
def gemini_json(
    ...
    """Single-turn Gemini call requesting JSON output."""
    from google.genai import types
...
def xstest_judge(prompt: str, response: str, model: str = "gemini-2.5-flash"):
    """Run the XSTest 3-way refusal judge. Returns (classification, raw_text).

    Never raises: a failed call yields ("judge_failed", "ERROR: ..."), so callers
    can count failures and exclude them from rates rather than losing the run.
    """
    try:
        raw = gemini_text(
        ...
        return "judge_failed", f"ERROR: {e}"
    return classify_xstest_response(raw), raw


def judge_records(records, judge_model, make_prompt, classify, desc, workers=None):
    """Judge each record concurrently, annotating it in place.

    Sets `judge_raw` and `classification` on every record. make_prompt(rec) builds
    the judge prompt; classify(raw_text) turns the reply into a label. A failed
    call leaves judge_raw="" and lets `classify` decide the label for empty text.
    Returns the number of failures.
    """
    workers = JUDGE_WORKERS if workers is None else workers

    def _judge_one(rec):
        return gemini_text(make_prompt(rec), model=judge_model)

    fails = 0
    pbar = tqdm(total=len(records), desc=f"judge/{judge_model}/{desc}", unit="resp")
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futs):
            ...
            rec["classification"] = classify(judge_text)
            pbar.update(1)
            if fails:
