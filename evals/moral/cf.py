"""
Counterfactual consistency eval.
"""
...
from domains.moral.judges import MORAL_SYSTEM_PROMPT
from evals.manifest import get_checkpoint_dirs
...
# Gemini callers (shared evals.gemini module — Gemini only)
...
def make_judge_caller(model):
    ...
    return lambda prompt, response: xstest_judge(prompt, response, model=model)
...
# Generate counterfactuals from precomputed model rules
def build_cfs(
    explanations,
    gt_principles,
    json_caller,
...
# Generate responses for counterfactual prompts
...
def eval_responses_and_judge(
    ...
    cls_by, raw_by = {}, {}
    with ThreadPoolExecutor(max_workers=JUDGE_WORKERS) as pool:
        futs = {pool.submit(judge_caller, p, r): (side, k) for side, k, p, r in jobs}
        for fut in tqdm(
        ...
        ref_records[i].append(
            {
                "prompt": ref_flat[k],
                "rationale": (
                    cfs[i]["should_refuse_rationales"][pos]
                    if pos < len(cfs[i]["should_refuse_rationales"])
                    else ""
                ),
                "response": ref_resp[k],
                "classification": cls,
        ...
        com_records[i].append(
            {
                "prompt": com_flat[k],
                "rationale": (
                    cfs[i]["should_comply_rationales"][pos]
                    if pos < len(cfs[i]["should_comply_rationales"])
                    else ""
                ),
                "response": com_resp[k],
                "classification": cls,
...
def _variant_suffix(cf_model, n_prompts):
    ...
    # This port elicits one rule per category.
...
def run(
    ...
    print(
        f"Variant suffix: {suffix}  (cf_model={cf_model}, judge={judge_model}, n={n_prompts})"
    )
    ...
    def _write_snapshot(step, snap_dir, metrics, records):
        ...
        print(
            f"  [step {step}] refuse={metrics['refuse_accuracy']:.4f} "
