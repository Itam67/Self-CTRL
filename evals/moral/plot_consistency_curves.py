"""Consistency (jury eval score) over training"""
...
from evals.figure_utils import apply_serif_font
from evals.plot_style import PALETTE, apply_style
from evals.moral.viz_utils import (
    load_all_snapshots,
...
def _draw_panel(ax, run_dirs, xmax=None):
    ...
    if val_x:
        ...
        series.append(
            (val_x, val_y, PALETTE["cons"], f"Validation  (n={n_val})", False)
        )
    if holdout_snaps:
        ...
        if bx:
            n_b = _set_size(
                holdout_snaps, lambda r: r.get("holdout_type") == "broad_category"
            )
            series.append(
                (bx, by, PALETTE["held_cat"], f"Held-out category  (n={n_b})", True)
            )
        if fx:
            n_f = _set_size(
                holdout_snaps, lambda r: r.get("holdout_type") == "fine_principle"
            )
            series.append(
                (fx, fy, PALETTE["held_pri"], f"Held-out principle  (n={n_f})", True)
            )
...
def main():
    ...
    if missing:
        raise SystemExit(
            "No eval_snapshots/ found for panel(s): "
            + ", ".join(missing)
    ...
    fig, axes = plt.subplots(
        1, len(panels), figsize=(5.4 * len(panels), 5), sharey=True
    )
