...
Single panel; per manifest condition two bars: Refusal Acc (solid) and
Compliance Acc (hatched, relaxed), from the per-category cf eval
(cf_consistency_*_percat.json, Gemini judge).
...
this script ensures
    ...
    <results_dir>/cf_consistency_<cf_model_slug>_n<N>_percat.json

exists (running evals/cf_consistency_eval.py only when it doesn't), then plots
its "metrics" block:
    refuse_accuracy          <- "Refusal Acc" bar
    comply_accuracy_relaxed  <- "Compliance Acc" bar
    n_refuse_prompts / n_comply_prompts  (optional, for bootstrap CI error bars)

Ported from sample_consistency/exps/moral/evals/plot_cf_consistency.py.

Usage:
    python -m evals.plot_cf                       # compute what's missing, plot
    python -m evals.plot_cf --plot-only           # never run an eval
    python -m evals.plot_cf --only "Base"         # just this condition's eval
"""
...
def _load_cf_metrics(run_dir):
    ...
    if not matches:
        raise FileNotFoundError(
            f"no cf_consistency_*_percat.json under {run_dir}"
        )
...
def main():
    ...
    def _ci_label_y(ax, x, v, nside):
        ...
        err = _prop_ci(v, nside)
        if err is None:
            return v + 0.012
        ax.errorbar(x, v, yerr=[[err[0]], [err[1]]], fmt="none",
                    ecolor="#2a2a2a", elinewidth=1.4, capsize=5, capthick=1.4, zorder=4)
        return v + err[1] + 0.012

    fig, ax = plt.subplots(figsize=(14, 6))
    for x, fill, edge, vr, vc, nr, nc in zip(xs, fills, edges, refuse, comply, refuse_n, comply_n):
        ax.bar(x - off, vr, w, facecolor=fill, edgecolor=edge, linewidth=BAR_LW, zorder=2)
        ax.bar(x + off, vc, w, facecolor=fill, edgecolor="white", hatch="////",
               linewidth=0, zorder=2)
        ax.bar(x + off, vc, w, facecolor="none", edgecolor=edge, linewidth=BAR_LW, zorder=3)
        yr = _ci_label_y(ax, x - off, vr, nr)
        yc = _ci_label_y(ax, x + off, vc, nc)
        ax.text(x - off, yr, f"{vr:.2f}", ha="center", va="bottom",
                fontsize=16, color="#2a2a2a")
        ax.text(x + off, yc, f"{vc:.2f}", ha="center", va="bottom",
                fontsize=16, color="#2a2a2a")
    ...
    legend = [
        ...
        Patch(facecolor="#9e9e9e", edgecolor="white", hatch="////", label="Compliance Acc"),
    ]
